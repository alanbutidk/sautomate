...
class SAExecutor:
    ...
    def _split_by_session(self):
        ...
        for parsed in self.parser.lines:
            if parsed.session not in sessions:
                sessions[parsed.session] = []
            sessions[parsed.session].append(parsed)
    ...
    def _run_shell(self, parsed: ParsedLine, session: str):
        if session == "default":
            ...
            if sys.platform == "win32":
                if session.lower() in ("powershell", "powershell.exe", "pwsh", "pwsh.exe"):
                    result = subprocess.run([session, "-Command", parsed.raw], shell=False)
                else:
